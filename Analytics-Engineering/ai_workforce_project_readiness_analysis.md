...
The recreated framework assumes data from the following business systems.

```text
HR Employee Data
→ Employee profile, role, education, experience, department, location

Timesheet Data
→ Employee hours, project hours, billable/non-billable hours, week/month

Project Data
→ Project name, client, project status, start date, end date, completion %

Payroll Data
→ Employee payroll cost, hourly cost, salary, billing rate

Skills Matrix
→ Tools, technologies, certifications, AI/data/cloud exposure

Client Data
→ Client name, industry, contract type, revenue, project portfolio

Training Data
→ Courses completed, AI certifications, internal training participation

Project Use Case Data
→ Manual tasks, automation candidates, AI Agent opportunities
```
...
```text
dim_employee
→ employee_id, employee_name, role, department, education_level, years_experience

dim_client
→ client_id, client_name, industry, account_manager, contract_type

dim_project
→ project_id, project_name, client_id, project_type, status, start_date, end_date

fact_timesheet
→ employee_id, project_id, work_date, hours_worked, billable_flag

fact_payroll
→ employee_id, payroll_month, salary_cost, hourly_cost, billing_rate

dim_skill
→ skill_id, skill_name, skill_category

bridge_employee_skill
→ employee_id, skill_id, proficiency_level, years_used, last_used_date

fact_training
→ employee_id, training_name, training_category, completion_date

fact_project_use_case
→ project_id, use_case_id, use_case_name, manual_effort_hours, automation_potential
```
